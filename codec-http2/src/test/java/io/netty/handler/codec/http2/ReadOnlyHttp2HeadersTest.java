...
public class ReadOnlyHttp2HeadersTest {
    ...
    public void notKeyValuePairThrows() {
        ReadOnlyHttp2Headers.trailers(new AsciiString[] { null }, false);
    }

    @Test
    public void nullHeaderNameNotChecked() {
        ReadOnlyHttp2Headers.trailers(new AsciiString[] { null, null }, false);
    }

    @Test(expected = Http2Exception.class)
    public void nullHeaderNameValidated() {
        ReadOnlyHttp2Headers.trailers(new AsciiString[] { null, new AsciiString("foo") }, true);
    }

    @Test(expected = IllegalArgumentException.class)
    public void pseudoHeaderNotAllowedAfterNonPseudoHeaders() {
        ReadOnlyHttp2Headers.trailers(new AsciiString[] {
                new AsciiString(":name"), new AsciiString("foo"),
    ...
    public void emtpyHeaderNameAllowed() {
        ReadOnlyHttp2Headers.trailers(new AsciiString[] { AsciiString.EMPTY_STRING, new AsciiString("foo") }, false);
    }
    ...
    public void testIsEmpty() {
        Http2Headers headers = ReadOnlyHttp2Headers.trailers(new AsciiString[] {}, false);
    ...
    private ReadOnlyHttp2Headers newServerHeaders() {
        return ReadOnlyHttp2Headers.serverHeaders(new AsciiString("200"), otherHeaders(), false);
    }

    private ReadOnlyHttp2Headers newClientHeaders() {
        return ReadOnlyHttp2Headers.clientHeaders(
                new AsciiString("meth"), new AsciiString("/foo"),
                new AsciiString("schemer"), new AsciiString("respect_my_authority"), otherHeaders(), false);
    }

    private ReadOnlyHttp2Headers newTrailers() {
        return ReadOnlyHttp2Headers.trailers(otherHeaders(), false);
    }
